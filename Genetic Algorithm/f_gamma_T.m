function Gamma = f_gamma_T(xi, eta, delta, Delta, Psi)
% ------------------------ Description ------------------------ %
%                                                               %
...
global c N_tot Pe T0

Lamda = 0.01;    
Gamma_d0_xi = zeros(1,  9);
...
K0 = 2*c*H(1, :) * (1-Lamda);    %K0 = K0(i)
b0 = -Delta./(Delta+(Delta/Pe).^2).*K0;

% ------------ Fourier Series ------------ %
M = 50;                      % Sufficient Fouriur highest index
for m = 1:M
    Km(m, :) = 2/pi*sin(m*pi*c)/m*H(1, :) * (1-Lamda); % Km = Km(m, i)
...
Gamma_h = sum(.5*K0-c*delta) + c + T0;

% ------------ Fourier Series ------------ %
for n = 1:M
    qn(n) = Pe^2/2*(1-sqrt(1+4*n^2*pi^2/Pe^2)); % qn = qn(n)
    C2(n) = 2*sin(n*pi*c) - (sum(bn(n, :).*(1+Delta/Pe^2)*n*pi+2*delta*sin(n*pi*c)));
